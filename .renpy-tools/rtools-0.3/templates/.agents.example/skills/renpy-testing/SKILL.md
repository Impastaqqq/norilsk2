...
### 1.1 Disabling and Skipping Testcases
By default, test cases are set to `enabled True`. To prevent a testcase from running during standard test runs, specify `enabled False` inside the block before test statements:

```renpy
# Disabling a specific testcase
testcase my_ignored_test:
    enabled False
    # Test statements here will be skipped during suite execution
```

#### Conditional Test Skipping
You can dynamically skip tests based on the operating platform or specific runtime variables using Ren'Py's boolean expressions:

```renpy
# Skip on mobile builds (PC/Mac/Linux only)
testcase not_on_mobile:
    enabled not renpy.mobile
    # Logic here...

# Run exclusively on Windows
testcase windows_only:
    enabled renpy.windows
    # Logic here...
```

*Note:* When explicitly targeting a single test suite by name via CLI (e.g., `renpy test suite_name`), Ren'Py marks the suite with the `only` flag which will execute all test cases in that suite regardless of `enabled False`. To observe test exclusion, run the full test suite (e.g. `python manage.py test global -s`).
...
- **State Isolation:** Always reset relevant model state (e.g., `character_repository`, `player.inventory`, `time_system`) at the beginning of each `testcase` to ensure test isolation.
...
```python
    testcase test_structural_inheritance:
        python:
            ...
            assert issubclass(Character, renpy.store.object), "Character must subclass renpy.store.object"
```
...
To programmatically test that the rollback mechanism works, call `renpy.checkpoint()` and `renpy.rollback()`. To prevent infinite execution loops during rollback, use a phase sentinel object subclassing `NoRollback`:
...
default test_character = Character("c01", "Alora")
...
```renpy
    testcase test_e2e_combat_button_clicks:
        # 1. Clear background transitions & mount screen
...
> [!TIP]
> **Token Budget Optimization:** Always prefer running tests with `-s` or `--silent` flag (e.g., `python manage.py test global -s`). This suppresses stdout output for passing tests and only prints failures and final outcome summary statistics, significantly reducing context token consumption.

```powershell
# Run a specific testsuite (silent mode recommended)
python manage.py test <testsuite_name> -s
...
python manage.py test --exclude-e2e -s
```

### Command Line Overrides
If you want to force execution of all test cases regardless of whether they are configured with `enabled False`, append the `--enable-all` flag to your direct Ren'Py CLI command:
```powershell
renpy.sh <path_to_game> test --enable-all -s
```

### Pre-execution Static Checks, Linting & Testing
Always run static checks, Ren'Py lint, and unit tests before pushing or after making code changes:
```powershell
...
#### Inline Ignore Directives (`# noqa`)
Static checkers support inline ignore annotations on any `.rpy` script line:
- `# noqa: asset-check` or `# asset-check: ignore` - Ignores asset file validation for that line.
- `# noqa: rollback-check` or `# rollback-check: ignore` - Ignores save/rollback variable check for that line.
- `# noqa` - Ignores all static checks on that line.
...
* **The Problem:** In headless Linux CI containers, `xvfb-run` creates a virtual frame buffer without a physical GPU. Standard Xvfb setups may experience three issues:
...
* **Resolution:** 
  1. Configure the graphic environment with Mesa DRI drivers, dummy audio, and explicit 1080p GLX server args in CI workflows:
     ...
     steps:
       ...
       - name: Run Automated Tests
         run: xvfb-run --auto-servernum --server-args="-screen 0 1920x1080x24 +extension GLX +render -noreset" python manage.py test global -s
     ```
  ...
  3. Configure `_test.timeout = 15.0` in the `setup:` block of spatial E2E UI testsuites to prevent slower shared CI container runners from exceeding Ren'Py's default 5.0s per-statement timeout.
